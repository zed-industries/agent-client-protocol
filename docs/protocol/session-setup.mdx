...
sequenceDiagram
    ...
    alt Creating new session
        Client->>Agent: session/new
        Note right of Agent: Create session context
        Note right of Agent: Connect to MCP servers
        Agent-->>Client: session/new response (sessionId)
    else Loading existing session
        Client->>Agent: session/load (sessionId)
        Note right of Agent: Restore session context
        Note right of Agent: Connect to MCP servers
